    ...
    public static partial class ShapeUtils
    {
        ...
        public struct TessellationOptions
        {
            ...
            public float MaxTanAngleDeviation // The maximum angle allowed (in radians) between tangents before a split happens
            {
                ...
                set
                {
                    m_MaxTanAngleDev = Mathf.Clamp(value, ShapeUtils.Epsilon, Mathf.PI * 0.5f);
        ...
        static void HandleNewSegmentJoining(PathDistanceForwardIterator pathIt, PathPatternIterator patternIt, JoiningInfo[] joiningInfo, float halfThickness, float[] segmentLengths)
        {
            ...
            if (pathIt.Closed)
            {
                ...
                if ((pathIt.CurrentSegment == 0) || (pathIt.CurrentSegment == pathIt.Segments.Count - 2))
                {
                    closing = ForeseeJoining(
                            ShapeUtils.PathSegmentAtIndex(pathIt.Segments, pathIt.Segments.Count - 2),
                            ShapeUtils.PathSegmentAtIndex(pathIt.Segments, 0),
            ...
            joiningInfo[1] = ForeseeJoining(
                    ShapeUtils.PathSegmentAtIndex(pathIt.Segments, pathIt.CurrentSegment),
                    ShapeUtils.PathSegmentAtIndex(pathIt.Segments, pathIt.CurrentSegment + 1),
            ...
            TessellationOptions tessellateOptions, JoiningInfo[] joiningInfo, float[] segmentLengths, float totalLength, int rangeIndex, List<Vector2> verts, List<UInt16> inds, List<Vector2[]> pats)
        {
            ...
            for (;;)
            {
                ...
                else if (result == PathDistanceForwardIterator.Result.NewSegment)
                {
                    ...
                    else AddSegment(ShapeUtils.PathSegmentAtIndex(pathIt.Segments, pathIt.CurrentSegment), pathIt.CurrentT, pathProps.Stroke.HalfThickness, null, pathIt.SegmentLengthSoFar, verts, inds);
                    HandleNewSegmentJoining(pathIt, patternIt, joiningInfo, pathProps.Stroke.HalfThickness, segmentLengths);
                }
        ...
        static JoiningInfo ForeseeJoining(BezierSegment end, BezierSegment start, float halfThickness, float endSegmentLength)
        {
            ...
            joinInfo.TanAtEnd = ShapeUtils.EvalTangent(end, 1.0f);
            joinInfo.NormAtEnd = Vector2.Perpendicular(joinInfo.TanAtEnd);
            joinInfo.TanAtStart = ShapeUtils.EvalTangent(start, 0.0f);
            ...
            {
                joinInfo.PosThicknessClosingPoint = ShapeUtils.IntersectLines(joinInfo.PosThicknessEnd, joinInfo.PosThicknessEnd + joinInfo.TanAtEnd, joinInfo.PosThicknessStart, joinInfo.PosThicknessStart + joinInfo.TanAtStart);
                joinInfo.NegThicknessClosingPoint = ShapeUtils.IntersectLines(joinInfo.NegThicknessEnd, joinInfo.NegThicknessEnd + joinInfo.TanAtEnd, joinInfo.NegThicknessStart, joinInfo.NegThicknessStart + joinInfo.TanAtStart);
            ...
            if (!joinInfo.SimpleJoin)
            {
                BezierSegment endFlipped = ShapeUtils.FlipSegment(end);
            ...
            {
                var intersect = ShapeUtils.IntersectLines(startTrail[0], startTrail[1], endTrail[0], endTrail[1]);
