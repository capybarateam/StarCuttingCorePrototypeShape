...
namespace ShapeGraphics
{
    public partial class ShapeUtils
    {
        ...
        {
            if ((p1-p0).magnitude < ShapeUtils.Epsilon)
            ...
            var path = ShapeUtils.MakeArc(Vector2.zero, theta1, sweepTheta, 1.0f);

            var scaling = new Vector2(adjustedRx, adjustedRy);
            path = ShapeUtils.TransformBezierPath(path, c, rotation, scaling);
