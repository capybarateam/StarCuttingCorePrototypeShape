...
using UnityEngine;
using ShapeGraphics.External.LibTessDotNet;
...
namespace ShapeGraphics
{
    public static partial class ShapeUtils
    {
        ...
        public class Geometry
        {
            ...
            /// <summary>The vertex paths of the geometry.</summary>
        ...
        private static void TessellateShape(Shape vectorShape, List<Geometry> geoms, TessellationOptions tessellationOptions, bool isConvex)
        {
            ...
            if (stroke != null && stroke.HalfThickness > ShapeUtils.Epsilon)
            {
                foreach (var c in vectorShape.Contours)
                {
                    ...
                    {
                        geoms.Add(new Geometry() { Vertices = strokeVerts, Indices = strokeIndices, Paths = strokePats, Color = vectorShape.PathProps.Stroke.Color });
                    }
                }
            }
        ...
        private static void TessellateConvexContour(Shape shape, Stroke stroke, Color color, List<Geometry> geoms, TessellationOptions tessellationOptions)
        {
            ...
            geoms.Add(new Geometry() { Vertices = vertices, Indices = indices, Paths = new Vector2[][] { paths }, Color = color, Fill = shape.Fill, FillTransform = shape.FillTransform });
        ...
        private static void TessellateShapeLibTess(Shape vectorShape, Color color, List<Geometry> geoms, TessellationOptions tessellationOptions)
        {
            ...
            if (indices.Count() > 0)
            {
                geoms.Add(new Geometry() { Vertices = vertices.ToArray(), Indices = indices.ToArray(), Paths = paths, Color = color, Fill = vectorShape.Fill, FillTransform = vectorShape.FillTransform });
            }
        ...
        private static void TessellatePath(BezierContour contour, PathProperties pathProps, List<Geometry> geoms, TessellationOptions tessellationOptions)
        {
            ...
            if (pathProps.Stroke != null)
            {
                ...
                if (indices.Length > 0)
                {
                    geoms.Add(new Geometry() { Vertices = vertices, Indices = indices, Paths = paths, Color = color });
                }
            }
