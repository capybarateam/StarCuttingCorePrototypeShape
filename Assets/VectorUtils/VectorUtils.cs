...
namespace ShapeGraphics
{
    /// <summary>
    /// Provides various tools to work with vector graphics.
    /// </summary>
    public static partial class ShapeUtils
    {
        ...
        public static BezierPathSegment[] BezierSegmentsToPath(BezierSegment[] segments)
        {
            ...
            for (int i = 0; i < segmentCount; ++i)
            {
                ...
                else
                {
                    ...
                    {
                        var line = ShapeUtils.MakeLine(seg.P3, nextSeg.P0);
        ...
        public static Rect Bounds(BezierPathSegment[] path)
        {
            ...
            foreach (var s in ShapeUtils.SegmentsInPath(path))
            {
        ...
        public static BezierPathSegment[] MakeArc(Vector2 center, float startAngleRads, float sweepAngleRads, float radius)
        {
            ...
            for (int quadrant = 0; quadrant <= endQuadrant; ++quadrant)
            {
                ...
                {
                    ShapeUtils.SplitSegment(seg, intersects[0], out subSeg1, out subSeg2);
                    seg = subSeg2;
                }
                ...
                if (!ShapeUtils.IsEmptySegment(seg))
                    segments.Add(seg);
            }
            ...
            if (shouldFlip)
            {
                ...
                for (int i = 0; i < segments.Count / 2; ++i)
                {
                    ...
                    var seg0 = ShapeUtils.FlipSegment(segments[i]);
                    var seg1 = ShapeUtils.FlipSegment(segments[j]);
                    segments[i] = seg1;
                    segments[j] = seg0;
                }
                if ((segments.Count % 2) == 1)
                {
                    ...
                    segments[i] = ShapeUtils.FlipSegment(segments[i]);
                }
            }

            return ShapeUtils.BezierSegmentsToPath(segments.ToArray());
        }
        ...
        internal static BezierSegment ArcSegmentForQuadrant(int quadrant)
        {
            switch (quadrant)
            {
                case 0: return ShapeUtils.MakeArcQuarter(Vector2.zero, 0.0f, Mathf.PI / 2.0f);
                case 1: return ShapeUtils.MakeArcQuarter(Vector2.zero, -Mathf.PI / 2.0f, Mathf.PI / 2.0f);
                case 2: return ShapeUtils.MakeArcQuarter(Vector2.zero, -Mathf.PI, Mathf.PI / 2.0f);
                case 3: return ShapeUtils.MakeArcQuarter(Vector2.zero, -Mathf.PI / 2.0f * 3.0f, Mathf.PI / 2.0f);
        ...
        static float MidPointQuadraticApproxLength(BezierSegment segment)
        {
            ...
            if (A1 != Vector2.zero)
            {
                ...
                if (Math.Abs(q) <= ShapeUtils.Epsilon)
                    return (float)l0;
        ...
        public static float[] FindBezierLineIntersections(BezierSegment segment, Vector2 p0, Vector2 p1)
        {
            ...
            foreach (var t in roots)
            {
                ...
                var s = 0.0f;
                if (Mathf.Abs(p1.x - p0.x) > ShapeUtils.Epsilon)
        ...
        private static float[] CubicRoots(double a, double b, double c, double d)
        {
            ...
            if (D >= 0)
            {
                ...
                if (Math.Abs(Im) > ShapeUtils.Epsilon)
                {
                    t[1] = -1;
                    t[2] = -1;
                }
    ...
    internal class PathDistanceForwardIterator
    {
        ...
        public PathDistanceForwardIterator(IList<BezierPathSegment> pathSegments, bool closed, float maxCordDeviationSq, float maxTanAngleDevCosine, float stepSizeT)
        {
            ...
            Segments = closed && !ShapeUtils.PathEndsPerfectlyMatch(pathSegments) ? new BezierLoop(pathSegments) : pathSegments;
            this.closed = closed;
            ...
            currentTTangent = needTangentsDuringEval ? ShapeUtils.EvalTangent(currentBezSeg, 0.0f) : Vector2.zero;
        }
        ...
        float PointToLineDistanceSq(Vector2 point, Vector2 lineStart, Vector2 lineEnd)
        {
            ...
            if (lineMagSq < ShapeUtils.Epsilon)
        ...
        public Result AdvanceBy(float units, out float unitsRemaining)
        {
            ...
            for (;;)
            {
                ...
                Vector2 point = needTangentsDuringEval ? ShapeUtils.EvalFull(currentBezSeg, nextT, out tangent) : ShapeUtils.Eval(currentBezSeg, nextT);
                ...
                if (!generateStepHere && (maxCordDeviationSq != float.MaxValue))
                {
                    ...
                    if (distPtToFirstSq > ShapeUtils.Epsilon)
                    {
                        Vector2 secondPoint = ShapeUtils.Eval(currentBezSeg, Mathf.Min((nextT - currentT) * 2.0f + currentT, 1.0f));
                ...
                if (dist > unitsRemaining)
                {
                    ...
                    dist = unitsRemaining;
                    point = ShapeUtils.Eval(currentBezSeg, nextT);
                }
        ...
        public IList<BezierPathSegment> Segments { get; set; }
        ...
        public Vector2 EvalCurrent() { return ShapeUtils.Eval(currentBezSeg, currentT); }
    ...
    internal class PathPatternIterator
    {
        public PathPatternIterator(float[] pattern, float patternOffset = 0.0f)
        {
            ...
            if (patternLength < ShapeUtils.Epsilon)
            {
