...
namespace ShapeGraphics
{
    public static partial class ShapeUtils
    {
        ...
        public static Sprite BuildSprite(List<Geometry> geoms, Rect rect, float svgPixelsPerUnit, Alignment alignment, Vector2 customPivot, UInt16 gradientResolution, bool flipYAxis = false)
        {
            ...
            if (rect == Rect.zero)
            {
                rect = ShapeUtils.Bounds(vertices);
                ShapeUtils.RealignVerticesInBounds(vertices, rect, flipYAxis);
            }
            else if (flipYAxis)
            {
                ShapeUtils.FlipVerticesInBounds(vertices, rect);
                ...
                ShapeUtils.ClampVerticesInBounds(vertices, rect);
            }
